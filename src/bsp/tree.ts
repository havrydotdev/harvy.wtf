import { newBspNode, type BspNode, type Rect, type WindowData, type Window } from "./node"
...
export function newBspTree(screenRect: Rect): BspTree {
    ...
    return {
        ...
        addWindow(window: Window) {
            ...
            this.current = node.insert(window, this.lowerRight ? 1 : 0, this.ratio)
        ...
        remove() {
            if (this.current.parent) {
                ...
                if (!newClient) {
                    ...
                    this.current = this.windows().filter((w) => w.node.nodeId == newClient.node.nodeId)[0].node
                }
            ...
            while (parent) {
                if (parent.splitHorizontal && parent.children[1]?.nodeId == child.nodeId) {
                    ...
                    while (!neighbor?.window) {
                        if (neighbor.splitHorizontal || (neighbor.children[1] && neighbor.children[1].rect.y < center)) {
        ...
        findUp() {
            ...
            while (parent) {
                if (!parent.splitHorizontal && parent.children[1]?.nodeId == child.nodeId) {
                    ...
                    while (!neighbor?.window) {
                        if (!neighbor.splitHorizontal || (neighbor.children[1] && neighbor.children[1].rect.x < center)) {
                            neighbor = neighbor.children[1]
                        } else { 
                            neighbor = neighbor.children[0]
                        }
